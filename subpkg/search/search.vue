<template>
  <view>
    <view class="search-box">
      <!-- 使用 uni-ui 提供的搜索组件 -->
      <uni-search-bar radius="15" :focus="true" v-model="searchValue" @input="input" cancelButton="none">
      </uni-search-bar>
    </view>
    <!-- 搜索建议列表 -->
    ...
    <view class="history-box" v-else>
      ...
      <view class="history-list">
        <uni-tag :circle="true" :text="item" v-for="(item, i) in historys" :key="i" @click="gotoGoodsList(item)">
        </uni-tag>
      </view>
    </view>
...
<script>
  export default {
    ...
    methods: {
      ...
      // 点击跳转到商品列表页面
      gotoGoodsList(searchValue) {
        uni.navigateTo({
          url: '/subpkg/goods_list/goods_list?query=' + searchValue
        })
      }
    },
    onBackPress() {
